...
namespace mlaskal {
    ...
    typedef lit_storage<int>		aic_literal_storage_int_type;
    /// real literal storage
    typedef lit_storage<double>		aic_literal_storage_real_type;
    /// string literal storage
    typedef lit_storage<std::string>    aic_literal_storage_string_type;
    /// identifier storage
    typedef lit_storage<std::string>    aic_literal_storage_id_type;
	...
	**/
	abstract_function_param(
	    aic_literal_storage_id_type::const_iterator name, 
	    physical_type ptype, 
	    ICABSNUM size, 
	    ICSTKOFFS stkoffs) 
	...
	/// parameter name
	aic_literal_storage_id_type::const_iterator get_name() const { return name_; }
	/// physical type (elementary)
	physical_type get_ptype() const { return ptype_; }
	...
	ICSTKOFFS get_stkoffs() const { return stkoffs_; }
    private:
	aic_literal_storage_id_type::const_iterator		name_;
	physical_type						ptype_;
	ICABSNUM						size_;
	ICSTKOFFS						stkoffs_;
    };
	...
	**/
	abstract_function_var(aic_literal_storage_id_type::const_iterator name,
			      physical_type ptype,
	...
	/// @cond FOR_MLC
	abstract_function(aic_literal_storage_id_type::const_iterator name,	// for a MLC function
			  const abstract_function_rv &rv,
			  const abstract_function_params &params,
			  const abstract_function_vars &vars,
			  tr1::shared_ptr<labeled_icblock> block) 
	...
	abstract_function(aic_literal_storage_id_type::const_iterator name,	// for a procedure
			  const abstract_function_params &params,
			  const abstract_function_vars &vars,
			  tr1::shared_ptr<labeled_icblock> block) 
			  : block_(block),name_(name),fs_type_(FNCSIG_PROC),rv_(abstract_function_rv()),params_(params),vars_(vars),
			    uid_(uid_all.get_UID()),other_uid_(0),used_regs_(0) { }
	abstract_function(aic_literal_storage_id_type::const_iterator name,	// for a program
			  const abstract_function_vars &vars,
			  tr1::shared_ptr<labeled_icblock> block) 
	...
	/// function code block
	tr1::shared_ptr<labeled_icblock> get_block() const { return block_; }
	...
	/// @cond FOR_MLC
	void assign_block(tr1::shared_ptr<labeled_icblock> block);
	/// @endcond 

    private:
	tr1::shared_ptr<labeled_icblock>			block_;
	aic_literal_storage_id_type::const_iterator		name_;
	...
	iterator find_name(aic_literal_storage_id_type::const_iterator name);
	/// find function by name
	const_iterator find_name(aic_literal_storage_id_type::const_iterator name) const;
