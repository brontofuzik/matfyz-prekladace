...
namespace mlaskal {
	...
	plain_icblock(const plain_icblock &pb)
	{
	    for(const_iterator ci=pb.block_.begin();ci!=pb.block_.end();++ci)
		block_.push_back(new T(**ci));
	}
	plain_icblock &operator=(const plain_icblock &pb)
	{
	plain_icblock tmp(pb);
