...
namespace {
    ...
    public:
	pred_find_name(::mlaskal::aic_literal_storage_id_type::const_iterator name) : name_(name) { }
	bool operator()(::mlaskal::abstract_functions::const_reference af) { return af.get_name()==name_; }
    private:
	::mlaskal::aic_literal_storage_id_type::const_iterator name_;
    };
	...
	{
	    ::mlaskal::tr1::shared_ptr< ::mlaskal::labeled_icblock> lic(af_->get_block());
	    return ::mlaskal::abstract_function_var(
				    aic_->get_ls_id().at_iter(fv.get_name()), 
				    fv.get_ptype(),
				    lic->rel_iter(lic->begin(), fv.get_icip_start()-fncsip_),
				    lic->rel_iter(lic->begin(), fv.get_icip_end()-fncsip_ + 1), // make it excluded end
				    fv.get_size(),
				    fv.get_stkoffs()
				    );
	}
	...
	::mlaskal::abstract_functions::value_type operator()(const ::mlaskal::file_function &ff)
	{
	    ...
	    return ::mlaskal::abstract_function(ff.get_type(), 
					...
					::mlaskal::tr1::shared_ptr< ::mlaskal::labeled_icblock>(static_cast< ::mlaskal::labeled_icblock*>(0))
				    );
	}
...
namespace mlaskal {
	...
	void operator()(const file_function &ff)
	{
	    ...
	    tr1::shared_ptr<labeled_icblock> licb(new labeled_icblock(
    ...
    void abstract_function::assign_block(tr1::shared_ptr<labeled_icblock> block)
    {
//	delete block_;
	block_ = block;
    }
    ...
    abstract_functions::iterator abstract_functions::find_name(aic_literal_storage_id_type::const_iterator name)
    { 
	return find_if(fncs_.begin(), fncs_.end(), pred_find_name(name)); 
    }

    abstract_functions::const_iterator abstract_functions::find_name(aic_literal_storage_id_type::const_iterator name) const
    { 
	return find_if(fncs_.begin(), fncs_.end(), pred_find_name(name)); 
    }
