...
namespace mlc {
	...
	if ( nested_ )
	{
	    ...
	    my_function_code()->get_vars().push_back(
		...
		symbol_entry( SKIND_LOCAL_VARIABLE, lvarsize(), ltype)));
	...
	else
	{
		...
		symbol_entry( SKIND_GLOBAL_VARIABLE, gvarsize, ltype)));
	...
	for ( parameter_list::const_iterator pit = mit->second.parlist->begin(); pit != mit->second.parlist->end(); ++pit)
	{
	    ...
	    {
	    case PMODE_BY_VALUE:
		rv = locals().insert( symbol_map::value_type( pit->idx, symbol_entry( SKIND_LOCAL_VARIABLE, - defsize, pit->ltype)));
		defsize -= pit->ltype->compute_size();
		break;
	    case PMODE_BY_REFERENCE:
		rv = locals().insert( symbol_map::value_type( pit->idx, symbol_entry( SKIND_PARAMETER_BY_REFERENCE, - defsize, pit->ltype)));
		defsize -= 1;
		break;
	    case PMODE_UNDEF:	    // dazed GCC
		break;
